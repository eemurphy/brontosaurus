...
func clone(_ *cobra.Command, _ []string) {
	...
	err := filepath.Walk(".", func(path string, info os.FileInfo, err error) error {
		...
		if strings.Contains(dest, "brontosaurus") {
			dest = strings.Replace(dest, "brontosaurus", strings.ToLower(provisionCfg.Name), -1)
		}
		...
		if info.IsDir() {
			...
			if strings.Contains(content, "brontosaurus") {
				...
				content = strings.Replace(content, "brontosaurus", provisionCfg.Name, -1)
			}
			...
			if strings.Contains(content, "brontosaurus-db") {
				glog.Infof("find/replace required for file: %s", path)
				content = strings.Replace(content, "brontosaurus-db", strings.ToLower(provisionCfg.Name), -1)
			}
